...
from pyspark.sql.functions import lit, concat, when, to_date, round
...
doc1= sparks.read.load( "/home/Jadis/Documenti/bigData/daily-historical-stock-prices-1970-2018/historical_stock_prices.csv",\
...
#take the years>2004
doc= init.withColumn('anno',when(init.anno >= 2016, init.anno)).drop(init.anno)
...
print("join1")
a= join1.groupBy([join1.name,join1.sector,join1.anno])\
      .agg(F.sum(join1.close))\
      .toDF("name", "sector","anno","close_min")
...
print("join2")
b= join2.groupBy([join2.name,join2.sector,join2.anno])\
      .agg(F.sum(join2.close))\
      .toDF("name", "sector","anno","close_max")


join3=a.join(b,["name", "sector","anno"])\
      .withColumn("variazione_percentuale",F.round((b.close_max-a.close_min)/a.close_min *100))\
      .drop(b.close_max).drop(a.close_min).drop(b.name)\
      .toDF("name","sector","anno","variazione_percentuale")

join3nuovo= join3

finale=join3.join(join3nuovo)\
      .where((join3.name!=join3nuovo.name)\
      &(join3.sector!=join3nuovo.sector)\
      &(join3.anno==join3nuovo.anno)\
      &(join3.variazione_percentuale == join3nuovo.variazione_percentuale))
